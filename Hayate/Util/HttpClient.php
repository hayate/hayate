 ...
 */
/**
 * Updated by: Andrea Belvedere
 * Changes:
 * 1) updated for php 5
 * 2) changed $user_agent
 * 3) added head method
 * 4) return of getStatus() is casted to an "int"
 */

class Hayate_Util_HttpClient
{
    // Request vars
    protected $host;
    protected $port;
    ...
    protected $user_agent = 'Hayate HttpClient v1.0';
    // Options
    ...
    public function head($path, $data = false)
    {
        $this->path = $path;
        $this->method = 'HEAD';
        if ($data)
	...
	if (!$fp = @fsockopen($this->host, $this->port, $errno, $errstr, $this->timeout))
	{
	    // Set error message
            switch($errno) {
	    case -3:
		$this->errormsg = _('Socket creation failed (-3)');
	    case -4:
		$this->errormsg = _('DNS lookup failure (-4)');
	    case -5:
		$this->errormsg = _('Connection refused or timed out (-5)');
	    default:
		$this->errormsg = sprintf(_('Connection failed (%d)'), $errno);
